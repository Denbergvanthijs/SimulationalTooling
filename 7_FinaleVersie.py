...
class Car(object):
    def __init__(self, env, other, wachtrij, name, duurOp=5, lives=3):
        self.env = env
        ...
        self.lives = lives  # Aantal levens van deze auto
            ...
            with self.wachtrij.request() as req:
                ...
                if self.other.staat[0]:  # if groen/geels
                    ...
                    if not self.other.staat[0] and (self.env.now - self.other.staat[1]) > self.other.grens:  # if rood
                        self.lives -= 1
                        print(f"\x1b[0;31m\t{self.name}\x1b[0m tot \x1b[0;31m{self.env.now}\x1b[0m op kruispunt en heeft nu \x1b[0;31m{self.lives}\x1b[0m leven(s)")
                    else:
                        print(f"\x1b[0m\t{self.name} tot {self.env.now} op kruispunt")

                    if not self.lives:
                        print(f"\x1b[0;31m\t{self.name}\x1b[0m valt dus af")
